defmodule GoalmaticExAs1036Web.ChallengeHTML do
  ...
  attr :rest, :global,
    include: ~w(autocomplete name rel action enctype mathod novalidate target),
    ...
    <div class="flex flex-row justify-center items-center space-x-4 w-full my-12">
      <.form :let={f} for={@for} {@rest} class="">
        <div class="flex flex-row justify-center items-center space-x-4">
          ...
          <button
            type="submit"
            class="p-2.5 ml-2 text-sm font-medium text-white bg-indigo-700 rounded-lg border border-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800"
          >
            <svg
              class="w-4 h-4"
              fill="none"
